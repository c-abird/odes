...
cdef extern from "sundials_auxiliary/sundials_auxiliary.c":
    ...
    cdef N_VectorContent_Serial nv_content_s(N_Vector v)
    cdef long int nv_length_s(N_VectorContent_Serial vc_s)
    cdef booleantype nv_own_data_s(N_VectorContent_Serial vc_s)
    cdef realtype* nv_data_s(N_VectorContent_Serial vc_s)
    cdef realtype get_nv_ith_s(nv_content_data_s vcd_s, int i)
    cdef void set_nv_ith_s(nv_content_data_s vcd_s, int i, realtype new_value)

    ctypedef realtype *DlsMat_col
    cdef int get_dense_N(DlsMat A)
    cdef int get_dense_M(DlsMat A)
    cdef int get_band_mu(DlsMat A)
    cdef int get_band_ml(DlsMat A)
    cdef realtype* get_dense_col(DlsMat A, int j)
    cdef void set_dense_col(DlsMat A, int j, realtype *data)
    cdef realtype get_dense_element(DlsMat A, int i, int j)
    cdef void set_dense_element(DlsMat A, int i, int j, realtype aij)
    cdef DlsMat_col get_band_col(DlsMat A, int j)
    cdef void set_band_col(DlsMat A, int j, realtype *data)
    cdef realtype get_band_col_elem(DlsMat_col col_j, int i, int j)
    cdef void set_band_col_elem(DlsMat_col col_j, int i, int j, realtype aij)
    cdef realtype get_band_element(DlsMat A, int i, int j)
    cdef void set_band_element(DlsMat A, int i, int j, realtype aij)
